...
float V_GGX(float NdotL, float NdotV, float alphaRoughness) {
	...
	float GGXL =
		NdotV *
		sqrt(NdotL * NdotL * (1.0 - alphaRoughnessSq) + alphaRoughnessSq);
...
float lambdaSheen(float cosTheta, float alphaG) {
	if (abs(cosTheta) < 0.5) {
		...
		return exp(
			2.0 * lambdaSheenNumericHelper(0.5, alphaG) -
...
float V_Sheen(float NdotL, float NdotV, float sheenRoughness) {
	...
	return clamp(
		1.0 / ((1.0 + lambdaSheen(NdotV, alphaG) + lambdaSheen(NdotL, alphaG)) *
			   (4.0 * NdotV * NdotL)),
		0.0, 1.0);
}
...
//  https://github.com/KhronosGroup/glTF/tree/master/specification/2.0#acknowledgments
//  AppendixB
vec3 BRDF_specularGGX(
	float alphaRoughness, float NdotL, float NdotV, float NdotH) {
...
float D_GGX_anisotropic(
	float NdotH, float TdotH, float BdotH, float anisotropy, float at,
	float ab) {
	float a2 = at * ab;
...
float V_GGX_anisotropic(
	float NdotL, float NdotV, float BdotV, float TdotV, float TdotL,
	float BdotL, float at, float ab) {
...
vec3 BRDF_specularGGXAnisotropy(
	float alphaRoughness, float anisotropy, vec3 n, vec3 v, vec3 l, vec3 h,
	vec3 t, vec3 b) {
	...
	float V = V_GGX_anisotropic(
		NdotL, NdotV, dot(b, v), dot(t, v), dot(t, l), dot(b, l), at, ab);
	float D =
		D_GGX_anisotropic(NdotH, dot(t, h), dot(b, h), anisotropy, at, ab);
