...
vec3 getClearcoatNormal(NormalInfo normalInfo) {
    ...
    vec3 n =
        texture(u_ClearcoatNormalSampler, getClearcoatNormalUV()).rgb * 2.0 -
            vec3(1.0);
...
MaterialInfo getMetallicRoughnessInfo(MaterialInfo info) {
    ...
    vec4 mrSample =
        texture(u_MetallicRoughnessSampler, getMetallicRoughnessUV());
...
MaterialInfo getSheenInfo(MaterialInfo info) {
    ...
    vec4 sheenRoughnessSample =
        texture(u_SheenRoughnessSampler, getSheenRoughnessUV());
...
MaterialInfo getSpecularInfo(MaterialInfo info) {
    ...
    info.f0_dielectric =
        min(info.f0_dielectric * u_KHR_materials_specular_specularColorFactor *
                specularTexture.rgb,
            vec3(1.0));
    info.specularWeight =
...
MaterialInfo getTransmissionInfo(MaterialInfo info) {
    ...
    vec4 transmissionSample =
        texture(u_TransmissionSampler, getTransmissionUV());
...
MaterialInfo getIridescenceInfo(MaterialInfo info) {
    ...
    info.iridescenceFactor *=
        texture(u_IridescenceSampler, getIridescenceUV()).r;
    #endif

    #ifdef HAS_IRIDESCENCE_THICKNESS_MAP
    float thicknessSampled =
        texture(u_IridescenceThicknessSampler, getIridescenceThicknessUV()).g;
    float thickness =
...
MaterialInfo getDiffuseTransmissionInfo(MaterialInfo info) {
    ...
    info.diffuseTransmissionFactor *=
        texture(u_DiffuseTransmissionSampler, getDiffuseTransmissionUV()).a;
    #endif

    #ifdef HAS_DIFFUSE_TRANSMISSION_COLOR_MAP
    info.diffuseTransmissionColorFactor *=
        texture(
            u_DiffuseTransmissionColorSampler, getDiffuseTransmissionColorUV())
        .rgb;
    #endif
...
MaterialInfo getClearCoatInfo(MaterialInfo info, NormalInfo normalInfo) {
    ...
    vec4 clearcoatSampleRoughness =
        texture(u_ClearcoatRoughnessSampler, getClearcoatRoughnessUV());
...
MaterialInfo getAnisotropyInfo(MaterialInfo info, NormalInfo normalInfo) {
    ...
    info.anisotropicT = mat3(normalInfo.t, normalInfo.b, normalInfo.n) *
            normalize(vec3(direction, 0.0));
