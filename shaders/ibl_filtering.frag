...
float computeLod(float pdf) {
	...
	// https://cgg.mff.cuni.cz/~jaroslav/papers/2007-sketch-fis/Final_sap_0073.pdf
	float lod = 0.5 * log2(
						  6.0 * float(u_width) * float(u_width) /
...
vec3 filterColor(vec3 N) {
	...
	for (int i = 0; i < u_sampleCount; ++i) {
		...
		if (u_distribution == cLambertian) {
			...
			vec3 lambertian =
				textureLod(u_cubemapTexture, H, lod).rgb * u_intensityScale;
			...
			if (NdotL > 0.0) {
				...
				vec3 sampleColor =
					textureLod(u_cubemapTexture, L, lod).rgb * u_intensityScale;
...
void main() {
	...
	if (u_isGeneratingLUT == 0) {
		...
		vec3 texCoord = 0.5 * v_position + 0.5;
